                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="border p-4 rounded">
                                    ...
                                    <div class="login-separater text-center mb-4"> <span>{{ __('content.sign in with') }} {{ __('content.phone') }}</span>
                                        <hr>
                                    </div>
                                    <div class="form-body">
                                        <form class="row g-3" method="POST" action="{{ route('site.login.phone.store') }}">
                                            @csrf
                                            <div class="col-12">
                                                <label for="phone" class="form-label">{{ __('content.phone') }}</label>
                                                <input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone" placeholder="{{ __('content.enter')." ". __('content.phone') }}"  name="phone" value="{{ old('phone') }}" required autocomplete="off" autofocus>
                                                @error('phone')
